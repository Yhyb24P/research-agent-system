...
| Kimi Code | `kimi` 0.39.1, local native CLI | ACP server over stdio | SUPPORTED (live ACP capability) | SUPPORTED (MCP HTTP/SSE capability) | SUPPORTED (`--output-format stream-json`) | BLOCKED_AUTH_REQUIRED | BLOCKED_AUTH_REQUIRED | structured-message driver candidate |
